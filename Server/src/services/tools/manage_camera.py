...
@mcp_for_unity_tool(
    ...
    description=(
        ...
        "The ScreenCapture path needs Unity's built-in Screen Capture module (com.unity.modules.screencapture), "
        "which is optional and NOT forced as a dependency; if it is absent the capture falls back to camera "
        "rendering (a Camera must exist in the scene), so Screen Space - Overlay UI will be missing from the image. "
