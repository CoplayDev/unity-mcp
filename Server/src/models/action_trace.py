...
class EditorEvent(BaseModel):
    ...
    Design principles:
    - GlobalID is used for internal storage (cross-session stability)
    - MCP output only exposes target_instance_id and target_name (not GlobalID)
    - target_id field is optional and typically omitted from AI-facing output

    C# side JsonProperty mappings:
    - Sequence -> "sequence"
    - TimestampUnixMs -> "timestamp_unix_ms"
    - Type -> "type"
    - TargetInstanceId -> "target_instance_id" (resolved from GlobalID)
    - TargetName -> "target_name" (human-readable display name)
    ...
    target_id: Optional[str] = Field(None, description="Internal GlobalID (not exposed to AI)")
    target_instance_id: Optional[int] = Field(None, description="Unity InstanceID (for runtime object access, null if object not found)")
    target_name: Optional[str] = Field(None, description="Human-readable target name (GameObject name, or formatted ID if object not found)")
