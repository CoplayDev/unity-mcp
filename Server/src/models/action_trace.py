...
class EditorEvent(BaseModel):
    ...
    precomputed_summary: Optional[str] = Field(None, description="Pre-computed summary generated by C# side")
    is_dehydrated: bool = Field(False, description="Whether event payload is dehydrated (memory optimization)")

    # Optional semantic fields (returned when include_semantics=True)
    ...
    # Optional context fields (returned when include_context=True)
    has_context: Optional[bool] = Field(None, description="Whether associated context information exists")
    context: Optional[dict[str, Any]] = Field(None, description="Context details")
...
class EventQueryResult(BaseModel):
    ...
    context_mapping_count: Optional[int] = Field(None, description="Context mapping count")
