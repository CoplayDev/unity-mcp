...
- **Disabled tool groups**: only the `core` group is enabled by default. `manage_tools(action="list_groups")` shows the groups, `manage_tools(action="activate", group="...")` enables one.
